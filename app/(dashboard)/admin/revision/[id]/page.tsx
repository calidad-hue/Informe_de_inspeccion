...
export default async function AdminRevisionDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const editable = inspeccion.status === "en_revision";
  ...
  return (
    <div className="space-y-6">
      ...
      {inspeccion.status === "aprobado" ? (
        <Link
          href={`/api/inspecciones/${id}/pdf`}
          className="inline-block min-h-11 leading-[2.75rem] rounded-md bg-carbon text-white px-6 hover:bg-carbon/90"
        >
          Descargar PDF
        </Link>
      ) : null}
